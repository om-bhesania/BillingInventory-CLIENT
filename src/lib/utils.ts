...
export const formatInvoiceForPrint = (formData: any) => {
  const subtotal = formData.items.reduce((t: number, i: any) => t + i.price * i.quantity, 0);
  ...
  // Bill Details
  const billNo = formData.invoiceNumber || `INV-${Date.now().toString().slice(-6)}`;
  const date = new Date(formData.invoiceDate).toLocaleDateString('en-IN', {
    day: '2-digit',
    month: 'short',
    year: 'numeric'
  }).replace(/\//g, ' - ');
  ...
  // Items Table Header
  receipt += "SN".padEnd(4) + "Item".padEnd(20) + "Qty".padEnd(6) + "Price".padEnd(10) + "Amt\n";
  receipt += "--------------------------------\n";
  ...
  receipt += `Subtotal`.padEnd(20) + 
             totalQty.toString().padEnd(6) + 
             `₹${subtotal.toFixed(2)}\n`;
  
  receipt += `IGST at 18%`.padEnd(20) + 
             `₹${tax.toFixed(2)}\n`;
